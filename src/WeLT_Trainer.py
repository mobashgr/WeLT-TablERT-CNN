...
class WeLTrainer(BaseTrainer):
    ...
    def train(self, train_path: str, valid_path: str, types_path: str, input_reader_cls: BaseInputReader):
        ...
        train_dataset, train_types_count, train_types_countr = input_reader.read(train_path, train_label)
        valid_dataset, valid_types_count, valid_types_countr = input_reader.read(valid_path, valid_label)
        orderedentitylabels,orderedrelationabels = self._log_datasets(input_reader)
        print("Train relation labels from log")
        print(orderedrelationabels)
        #orderedtrain_types_count = {x: train_types_count[x] for x in orderedentitylabels}
        #print("Train ordered types")
        #print(orderedtrain_types_count)
        orderedtrain_types_countr = {x: train_types_countr[x] for x in orderedrelationabels}
        print("Train relation types")
        print(orderedtrain_types_countr)
        #orderedvalid_types_count = {x: valid_types_count[x] for x in orderedentitylabels}
        orderedvalid_types_countr = {x: valid_types_countr[x] for x in orderedrelationabels}
        labelsum = sum(train_types_count.values())
        print("Total Tags")
        print(labelsum)
        rescalledentities = np.array([])
        for key, value in train_types_count.items():
            rescalledentities = np.append(rescalledentities, 1.0 - (value / labelsum))
        torchweightsr = torch.from_numpy(rescalledentities).float().to('cuda')
        rescaledweightsr = torch.softmax(torchweightsr, dim=0)
        print("Normalized rescaled weights")
        print((rescaledweightsr))
        print("Valid ordered relation types")
        print(valid_types_count)

        sum_relations = sum(orderedtrain_types_countr.values())
        print("Total relations")
        print(sum_relations)
        rescalledentitiesr = np.array([])
        for key, value in orderedtrain_types_countr.items():
            rescalledentitiesr = np.append(rescalledentitiesr, 1.0 - (value / sum_relations))
        torchweights_rel = torch.from_numpy(rescalledentitiesr).float().to('cuda')
        rescaledweights_rel = torch.softmax(torchweights_rel, dim=0)
        print("Rescalled")
        print((rescaledweights_rel))
        #print("Valid ordered types")
        #print(orderedvalid_types_countr)
        ...
        rel_criterion = torch.nn.CrossEntropyLoss(weight=rescaledweights_rel,reduction='none')
        #entity_criterion = torch.nn.CrossEntropyLoss(weight=rescaledweightsr, reduction='none')
        ...
        for epoch in range(args.epochs):
            ...
            self._train_epoch(model, compute_loss, optimizer, train_dataset, updates_epoch, epoch,
                              input_reader.entity_label_count, input_reader.relation_label_count)

            # eval validation sets
            if not args.final_eval or (epoch == args.epochs - 1):
                ner_acc, rel_acc, rel_ner_acc = self._eval(model, compute_loss, validation_dataset, input_reader, epoch,
                                                           updates_epoch)
                if args.save_best:
                    ...
                    self._save_best(model=model, optimizer=optimizer if self.args.save_optimizer else None,
                                    accuracy=ner_acc[2], iteration=epoch * updates_epoch, label='ner_micro_f1',
                                    extra=extra)
    ...
    def eval(self, dataset_path: str, types_path: str, input_reader_cls: BaseInputReader):
        ...
        # read datasets
        input_reader = input_reader_cls(types_path, self._tokenizer, self._logger)
        test_dataset, test_types_count,test_types_countr = input_reader.read(dataset_path, dataset_label)
        orderedentitylabels,orderedrelationabels = self._log_datasets(input_reader)
        #orderedtypes_count = {x: test_types_count[x] for x in orderedentitylabels}
        orderedtypes_countr = {x: test_types_countr[x] for x in orderedrelationabels}
        #print("Eval-orderedtypes")
        #print(orderedtypes_count)
        labelsum = sum(test_types_count.values())
        print("Total tags")
        print(labelsum)
        rescalledentities = np.array([])
        for key, value in test_types_count.items():
            rescalledentities = np.append(rescalledentities, 1.0 - (value / labelsum))
        torchweight_entities = torch.from_numpy(rescalledentities).float().to('cuda')
        rescaledweights_entities = torch.softmax(torchweight_entities, dim=0)
        print("Rescalled Entity Weights")
        print((rescaledweights_entities))

        print("Eval-orderedtypesr")
        print(test_types_countr)
        sum_relations = sum(test_types_countr.values())
        print("Total")
        print(sum_relations)
        rescalledrelations = np.array([])
        for key, value in orderedtypes_countr.items():
            rescalledrelations = np.append(rescalledrelations, 1.0 - (value / labelsum))
        torchweights_rel = torch.from_numpy(rescalledrelations).float().to('cuda')
        rescaledweights_rel = torch.softmax(torchweights_rel, dim=0)
        print("Rescalled Relations")
        print((rescaledweights_rel))
        # create model
        model = self._load_model(input_reader)
        model.to(self._device)
        ...
        # WeLT Loss function
        rel_criterion = torch.nn.CrossEntropyLoss(weight=rescaledweights_rel,reduction='none')
        entity_criterion = torch.nn.CrossEntropyLoss(weight=rescaledweights_entities, reduction='none')
    ...
    def _train_epoch(self, model: torch.nn.Module, compute_loss: Loss,
        ...
        for batch in tqdm(data_loader, total=total, desc='Train epoch %s' % epoch):
            ...
            loss = compute_loss.compute(ent_logits, batch['ent_labels'], rel_logits, batch['rel_labels'],
                                        batch['token_ctx_masks'])
    ...
    def _eval(self, model: torch.nn.Module, compute_loss: Loss, dataset: Dataset, input_reader: JsonInputReader,
        ...
        data_loader = DataLoader(dataset, batch_size=self.args.eval_batch_size, shuffle=False,
                                 drop_last=False, num_workers=self.args.sampling_processes,
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(data_loader, total=total, desc='Evaluate epoch %s' % epoch):
                ...
                loss = compute_loss.compute(ent_logits, batch['ent_labels'], rel_logits, batch['rel_labels'],
                                            batch['token_ctx_masks'], is_eval=True)
    ...
    def _get_optimizer_params(self, model):
        ...
        optimizer_params = [
            ...
            {'params': [p for n, p in params if any(nd in n for nd in no_decay) and not 'bert' in n],
             'weight_decay': 0.0},
    ...
    def _log_datasets(self, input_reader):
        ...
        for e in input_reader.entity_labels.values():
            Orderdlabels.append(e.short_name)
            self._logger.info(e.short_name + '=' + str(e.index))

        self._logger.info("Relations:")
        for r in input_reader.relation_labels.values():
            Orderdlabelsr.append(r.short_name)
        ...
        print("Orderedlabels:Entities")
        print(Orderdlabels)
        print("Orderedlabels:Relations")
        print(Orderdlabelsr)
        return Orderdlabels,Orderdlabelsr
