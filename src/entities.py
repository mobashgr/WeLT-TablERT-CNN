...
class RelationLabel:
    ...
    def __eq__(self, other):
        if isinstance(other, RelationLabel):
...
class RelationType:
    # General type of relation containing different labels. LocIn: ['L-LocIn', 'R-LocIn']
...
class EntityLabel:
    ...
    def __eq__(self, other):
        if isinstance(other, EntityLabel):
            return self._identifier == other._identifier
        return False
...
class EntityType:
    ...
    def __repr__(self):
        print("What is inside entities")
        print(self._labels)
        return f"{self._labels}"
...
class Entity:
    def __init__(self, eid: int, entity_type: EntityType, entity_labels: List[EntityLabel],tokens: List[Token], phrase: str):
    ...
    def __str__(self):
        return self._entity_type._labels
    def __repr__(self):
        return f"{self._entity_type._labels}" 
...
class Relation:
    ...
    @property
    def relation_typeshort(self):
        return self._relation_type._short_name
    ...
    def __repr__(self):
        return f"{self._relation_type._short_name}" 
...
class Dataset(TorchDataset):
    ...
    def __init__(self, label, rel_types, entity_types):
        ...
        # max sequence length (in subwords)
        self._max_seq_len = 0
    ...
    def create_document(self, tokens, entity_mentions, relations, doc_encoding) -> Document:
        ...
        if len(tokens) > self._max_sentence_len:
            self._max_sentence_len = len(tokens)
       
        if len(doc_encoding) > self._max_seq_len:
            self._max_seq_len = len(doc_encoding)
        return document
    ...
    def __getitem__(self, index: int):
        ...
        return sampling.create_sample(doc)
